...
resar1car= readRDS("car_beijing_2_40000.rds")


pred_res_car = readRDS("results_pred_car.rds")
...
predicommon = apply(resar1car$betaF,1,pred_common,x = xtot)
...
pred_car = apply(pred_res_car,1,quantile,probs = c(0.025,0.5,0.975))
pred_car = t(pred_car)

datapreinterest$ypred[datapreinterest$indipred==1] = pred_car[,2]
ycens = apply(resar1car$ycensF,2,mean)
datapreinterest$ypred[datapreinterest$indicens3==1 ] = ycens

datapreinterest$ypred_25[datapreinterest$indipred==1] = pred_car[,1]
datapreinterest$ypred_975[datapreinterest$indipred==1] = pred_car[,3]
